  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      try {
        ...
        const res = await axios.get('http://localhost:5003/api/suppliers', {
  ...
  {categories.map(category => (
    ...
    >
      {category.name}
        ...
        {/* Suppliers List from Backend */}
        {error ? (
          <div style={{ marginTop: '2rem', color: 'red', textAlign: 'center' }}>
            <div>{error}</div>
            {error.includes('log in') && (
              <button className="primary-btn" style={{ marginTop: '1rem' }} onClick={() => navigate('/SignInPage')}>
                Go to Login
              </button>
            )}
          </div>
          ...
          <div style={{ marginTop: '2rem' }}>
            <h2 style={{ color: '#441752', fontSize: '22px', marginBottom: '1rem' }}>All Suppliers</h2>
            <ul style={{ listStyle: 'none', padding: 0 }}>
              {suppliers.map(supplier => (
                <li key={supplier._id} style={{ background: '#fff', marginBottom: '0.5rem', padding: '1rem', borderRadius: '8px', color: '#441752', border: '1px solid #A888B5' }}>
                  <strong>{supplier.name}</strong> {supplier.type ? `- ${supplier.type}` : ''}<br/>
                  {supplier.email && <span>Email: {supplier.email}</span>}
                </li>
              ))}
              {suppliers.length === 0 && <li style={{ color: '#441752' }}>No suppliers found.</li>}
            </ul>
          </div>
        )}
      </main>
